...
export default function InvoicesOut() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [invoicesRes, partnersRes, categoriesRes] = await Promise.all([
        api.get('/invoices-out', { params: filters }),
        api.get('/partners', { params: { type: 'vendor' } }),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingInvoice) {
        await api.put(`/invoices-out/${editingInvoice.id}`, formData);
        toast.success('Invoice updated successfully');
      } else {
        await api.post('/invoices-out', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/invoices-out/${id}`);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="page-title">{t('invoicesOut')}</h1>
          <p className="page-subtitle">Manage expense invoices</p>
        </div>
              ...
              <tbody>
                {invoices.map((invoice) => (
                  <tr key={invoice.id}>
                    ...
                    <td className="font-bold text-red-600">{Number(invoice.amount).toLocaleString()}</td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-2xl font-bold mb-4">
              {editingInvoice ? 'Edit' : 'Add'} Expense Invoice
            </h2>
